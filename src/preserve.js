...
async function fetchLibraryHubs(sectionKey) {
	...
	for (const hub of hubsNodes) {
		...
		const seed =
			extractQueryParam(hub?.key, 'curatedHubSeed') ||
			normalize(hub?.curatedHubSeed) ||
			normalize(hub?.hubGuid) ||
			normalize(hub?.seed) ||
			normalize(hub?.hubSeed);
...
async function findHubInLibrary(sectionKey, hubTitle) {
	const { hubs } = await fetchLibraryHubs(sectionKey),
		target = String(hubTitle).trim().toLowerCase();
